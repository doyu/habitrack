...
Status: v4 — implemented and deployed (https://habitrack.ninjalabo.ai).
...
- **Name constraints** (a violation corrupts data or breaks routing, so
  ...
  no `/` (percent-encoding cannot survive path routing for `/`), no empty
  or duplicate names. Other URL metacharacters (`?` `#` `%` `&` spaces)
  are fine — `toggle_url` percent-encodes them.
- **Date constraints**: index cells must be ISO dates (`YYYY-MM-DD`),
  unique; a malformed date makes `load_full` raise (the page 500s until
  the row is fixed).
